...
struct Node{
    ...
    Node(int splitNumTemp, int splitFiTemp, int maxClassTemp, int treeLevelTemp, long long uniqueTemp){
...
//计算GINI指数
double get_gini_val(vector<vector<int>* > data, int featureTemp, int* FiTemp){
    map<int, double> giniNow;
    ...
    for(int FiLoop=0; FiLoop<d_feature_numb[featureTemp].size(); FiLoop++){
        ...
        //找到该子节点每个类所含数据数量
        map<int, int> nodeClassNum, nodeNonFiClassNum;
        for(int iData=0; iData<data.size(); iData++){

            if((*(data[iData]))[featureTemp] == Fi) {
                map<int, int>::iterator nodeClassNumIterator = nodeClassNum.find((*(data[iData]))[targetCol]);

                if (nodeClassNumIterator == nodeClassNum.end())
                    nodeClassNum[(*(data[iData]))[targetCol]] = 1;
                else
                    nodeClassNum[(*(data[iData]))[targetCol]] += 1;
            }
            else{
                map<int, int>::iterator nodeNonFiClassNumIterator = nodeNonFiClassNum.find((*(data[iData]))[targetCol]);

                if (nodeNonFiClassNumIterator == nodeNonFiClassNum.end())
                    nodeNonFiClassNum[(*(data[iData]))[targetCol]] = 1;
                else
                    nodeNonFiClassNum[(*(data[iData]))[targetCol]] += 1;
            }
        }

        double p = 0.0, pNonFi = 0.0;
        for(map<int, int>::iterator nodeClassNumIterator=nodeClassNum.begin(); nodeClassNumIterator!=nodeClassNum.end(); nodeClassNumIterator++){
            p += pow((nodeClassNumIterator->second*1.0 / dataFiNumb), 2);
        }
        for(map<int, int>::iterator nodeNonFiClassNumIterator=nodeNonFiClassNum.begin(); nodeNonFiClassNumIterator!=nodeNonFiClassNum.end(); nodeNonFiClassNumIterator++){
            pNonFi += pow((nodeNonFiClassNumIterator->second*1.0 / (data.size()-dataFiNumb)), 2);
        }
        giniNow[Fi] = dataFiNumb*1.0/data.size()*(1 - p) + (data.size()-dataFiNumb)*1.0/data.size()*(1-pNonFi);
    }
    double minGini = 0x7fffff;
    for(map<int, double>::iterator giniNowIterator=giniNow.begin(); giniNowIterator!=giniNow.end(); giniNowIterator++){
        if(giniNowIterator->second < minGini){
            minGini = giniNowIterator->second;
            *FiTemp = giniNowIterator->first;
        }
    }
    return minGini;
}
...
Node* build_tree(int treeLevel, bool if_add_stra, bool if_print=true){
	...
	while(!nodeQueue.empty()){
		...
		map<int, pair<int, double> > giniList;
		...
		for(int iFeature=0; iFeature<l_feature.size(); iFeature++){
            ...
            if((kv1Iterator==d_kv_fd.end() && kv2Iterator==d_kv_fd_2.end()) ||
                ...
                giniNow = get_gini_val(nodeTemp->nodeData, l_feature[iFeature], &FiTemp);
            ...
            giniList[l_feature[iFeature]].first = FiTemp;
            giniList[l_feature[iFeature]].second = giniNow;
        ...
        int minFeature = 0, minFi = 0;
        double minGini = (numeric_limits<double>::max)();
        for(map<int, pair<int, double> >::iterator giniIterator=giniList.begin(); giniIterator!=giniList.end(); giniIterator++){
            if(giniIterator->second.second < minGini){
                minGini = giniIterator->second.second;
                minFeature = giniIterator->first;
                minFi = giniIterator->second.first;
            }
        }
        if(if_print)
        ...
        nodeTemp->splitFi = minFi;
        vector<vector<int>* > subFiNodeData, subNonFiNodeData;
        for(int iData=0; iData<nodeTemp->nodeData.size(); iData++){
            if((*(nodeTemp->nodeData[iData]))[minFeature] == minFi)
                subFiNodeData.push_back(nodeTemp->nodeData[iData]);
            else
                subNonFiNodeData.push_back(nodeTemp->nodeData[iData]);
        }
        if(!subFiNodeData.empty()){
            ...
            (nodeTemp->subNode)[minFi] = subNodeTemp;
            nodeQueue.push(subNodeTemp);
        }
        if(!subNonFiNodeData.empty()){
            Node* subNodeTemp = &(treeNode[treeNodeIndex++]);
            subNodeTemp->nodeData = subNonFiNodeData;
            subNodeTemp->maxClass = set_class(subNonFiNodeData);
            subNodeTemp->treeLevel = nodeTemp->treeLevel+1;
            subNodeTemp->unique = treeNodeIndex-1;
            (nodeTemp->subNode)[minFi+1] = subNodeTemp;
            nodeQueue.push(subNodeTemp);
        }
	}
    return root;
...
Node* copy_tree(Node* root){
    Node* rootTemp = new Node(root->splitNum, root->splitFi, root->maxClass, root->treeLevel, root->unique);
    ...
    while(!queueNode.empty()){
        ...
        for(map<int, Node*>::iterator it=temp->subNode.begin(); it!=temp->subNode.end(); it++){
            ...
            Node* nodeTemp = new Node((*(it->second)).splitNum, (*(it->second)).splitFi, (*(it->second)).maxClass, (*(it->second)).treeLevel, (*(it->second)).unique);
...
double classify(Node* root){
    ...
    for(int iData=0; iData<test_data.size(); iData++){
        ...
        while(temp->splitNum != -1){
            if(temp->subNode[temp->splitFi] == NULL || temp->subNode[temp->splitFi + 1] == NULL){
                break;
            }
            if(test_data[iData][temp->splitNum] == temp->splitFi)
                temp = temp->subNode[temp->splitFi];
            else
                temp = temp->subNode[temp->splitFi + 1];
        }
...
void choose_best_tree(Node* root, queue<Node*> sub_tree_queue, bool stra){
